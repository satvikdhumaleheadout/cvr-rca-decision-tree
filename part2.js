...
const NODES = [
  ...
  },
  {
    ...
    description: 'Two-tier triage (see hypothesis.md). All queries filtered to fixed segment. Tier 1 — dimension cuts in parallel: language × S2C, device × S2C, experience × S2C, browsing_country (Geo/Non-Geo) × S2C. If experience concentrates: compute lost_checkouts_delta per TGID (users_select_post × Δs2c_rate) to identify locus — Case A (top TGID ≥60%), Case B (2–3 TGIDs ≥10% each), Case C (uniform drop). 3-step inventory path: Step 1 locus → Step 2 TID snapshot (Path A: pre >30d ago, post-only; Path B: pre within 30d, pre+post comparison) → Step 3 daily time-series (4 charts, one per bucket). Supply gate: if Step 2 shows no depleted limited-capacity TIDs, skip Step 3 and pivot to pricing/UX. Tier 2: if no concentration — check platform-wide availability config or checkout flow change. DRI: Supply/Inventory, Product, Ops, BDM.',
  },
  {
  ...
  },
  {
    ...
    fileKey: 'context',
    chips: ['Query Principles', 'Q3 Trend Interpretation', 'Mix Cascade queries', 'mix/conv effect arithmetic', 'Geo vs Non-Geo query (c013)', 'canonical L2+ template', 'inventory 3-step path (c015)', 'Path A vs Path B', 'supply gate', 'PERF_MAX filter (c012)'],
  },
  {
...
**Path A vs Path B — determine before running any query:**
- **Path B** (\`pre_start >= CURRENT_DATE - 30\`): pre-period rows available — run full pre/post comparison.
- **Path A** (\`pre_start < CURRENT_DATE - 30\`): pre-period rows unavailable — post-only snapshot with explicit data-limitation note in report.

**3-step inventory analysis (c015):**

**Step 1 — Locus:** From Q4 results, compute \`lost_checkouts_delta = users_select_post × (s2c_pre − s2c_post)\` per TGID. Three cases: **Case A** (top TGID ≥60% → single locus), **Case B** (2–3 TGIDs ≥10% each → multiple loci, run Step 2 for each), **Case C** (no TGID ≥10% → uniform drop, broad-drop path: run Step 2 for top 3 TGIDs by \`users_select\`).

**Step 2 — TID snapshot:** Latest \`extracted_date\` only. One row per TID. Ticket counts (sum of \`total_remaining\`) in four buckets: 0–2d, 3–7d, 8–13d, 14–30d. TIDs with \`is_fully_unlimited_capacity = TRUE\` excluded from supply scarcity analysis. **Supply gate:** if no limited-capacity TID shows depletion → supply is not the mechanism; pivot to pricing/UX.

**Step 3 — Daily time-series:** \`extracted_date\` trend per bucket. Path B: pre+post overlaid. Path A: post only. Scope to single TID (if one TID is the locus) or whole TGID (if all TIDs depleted equally). Feeds four line charts in report.
...
| c015 | 2026-05-06 | Inventory overhaul: removed count_days_available_30d proxy; 3-step path (locus → TID snapshot → time-series); Path A/B; supply gate; Case A/B/C locus logic; unlimited capacity guard |
`,
...
- \`experience_id\` × S2C — specific experiences → supply/pricing. If concentrates: compute \`lost_checkouts_delta\` per TGID → locus (Case A/B/C) → 3-step inventory path (context.md).
...
If experience concentrates: compute \`lost_checkouts_delta\` per TGID, identify Case A/B/C locus, then run Step 2 TID snapshot. Supply gate applies: no depleted limited-capacity TIDs → pivot to pricing/UX, do not run Step 3.
