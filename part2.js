...
const NODES = [
  ...
  },
  {
    ...
    description: 'Three-tier triage (see hypothesis.md). Tier 1: run dimension cuts in parallel — device × LP2S, language × LP2S, page_type × LP2S, experience × LP2S. If a dimension concentrates, drill to intersection then to page_url (the target output). Tier 2: if no concentration, run pricing analysis — final_price_usd pre/post for top experiences. Tier 3: if pricing is also flat, session recordings on the LP are the primary tool. DRI: Product, BDM, Performance Marketing, or Content depending on mechanism.',
  },
  {
  ...
  // ══ L1: MIX CASCADE ══
  {
    ...
    label: 'L1 — Mix Cascade', sublabel: 'MB/HO → Paid/Organic → Channel (mandatory)',
    x: 795, y: 2690, phase: 'Investigation',
    badge: null,
    inputs: ['summary.json (mb_share, ho_share)', 'ce_id', 'pre/post dates', 'is_microbrand_page fix from L0'],
    outputs: ['fixed_segment declaration', 'filter strings for all L2+ queries'],
    condition: 'Always fires on the funnel path, before any L2 hypotheses.\nMandatory even if mix was not dominant at L0.',
    description: 'Three-level cascade that fixes the primary segment before any funnel analysis. Level 1 (MB vs HO): read from summary.json — no query needed. Level 2 (Paid vs Organic within fixed brand): custom BQ query, group by channel type. Level 3 (Channel within Paid): only if Paid was fixed at Level 2 — which specific paid channel dominates? Fix a level when one segment has >15% of post users AND its absolute checkout impact exceeds all others combined. After cascade, declare the fixed segment in the transcript: "Fixed segment: MB · Paid · Google Ads — AND is_microbrand_page = TRUE AND channel_name = \'Google Ads\'". Every L2+ query carries these filters.',
  },
  ...
  },
  {
    ...
    fileKey: 'context',
    chips: ['Query Principles', 'Q3 Trend Interpretation', 'Mix Cascade queries', 'inventory_availability schema', 'Dimensions guide', 'table schemas'],
  },
  {
    ...
    usedBy: ['hypothesis'],
    fileKey: 'hypothesis',
    chips: ['L0 routing map', 'LP2S 3-tier triage', 'S2C 2-tier triage', 'C2O 4+3 hypotheses', 'URL concentration', '10 historical patterns'],
  },
  {
  ...
  },
  {
    ...
    chips: ['90d chart in S1 (always)', 'positive CVR green variant', 'fixed segment banner', 'raw user counts mandatory', 'weekly x-axis ticks'],
  },
  {
...
const EDGES = [
  ...
  { id:'e21b', from:'mix-cascade',  to:'hypothesis',     type:'always',      label:'fixed segment declared'    },
...
const SECTION_LABELS = [
  ...
  { text: '⑯ HTML Report',         x: 30, y: 4722},
  { text: '⑰ Evaluation',          x: 30, y: 5012},
];
...
## Mix Cascade — Fixing the Primary Segment (NEW in c006)

Run as the first L1 step in every funnel investigation. Three levels:

**Level 1 — MB vs HO:** Read from \`summary.json\` (mb_share, ho_share) — no query needed. Fix \`is_microbrand_page = TRUE/FALSE\`.

**Level 2 — Paid vs Organic:** Custom BQ query grouping \`channel_name\` into Paid (Google Ads, Microsoft Ads, Facebook Ads, Affiliates) vs Organic. Fix the dominant type.

**Level 3 — Channel within Paid:** Only if Paid was fixed. Break out individual channel names. Fix the largest absolute checkout-impact channel.

**Decision rule:** Fix a level when one segment has >15% of post users AND its absolute checkout impact (|Δcvr| × post users) exceeds all others combined. If distributed, log "no dominant segment" and carry all values.

**Declare fixed segment before writing any further hypotheses:**
\`\`\`
Fixed segment: MB · Paid · Google Ads
Filters: AND is_microbrand_page = TRUE AND channel_name = 'Google Ads'
\`\`\`
Every L2+ query must carry these filters.
...
| # | Date | Change |
|---|------|--------|
| c002 | 2026-04-27 | Query Principles, Q3 Trend, Dimensions to Query, Session Recordings guidance added (moved from SKILL.md) |
...
| c009 | 2026-04-29 | Investigation patterns moved to hypothesis.md — context.md now owns domain vocab + schemas + query rules only |
`,
...
### Mix — first-pass
Query \`COUNT(DISTINCT user_id)\` by \`page_url\` pre vs post for the affected segment. Which URLs gained/lost volume? A paid campaign serving specific collection pages that stopped → those pages show a traffic drop. Marketing owns it.
...
- \`page_type\` × LP2S — Collection drop (not Experience) → browse-level friction
- \`experience_id\` × LP2S — few experiences → listing quality/price/availability

If a dimension concentrates: run intersection (e.g., French × mobile), drill to \`page_url\`. **The page URL is the target output.**
...
**Tier 3 — If pricing flat:** Session recordings — look for UX pattern affecting all users (slow load, broken carousel, changed CTA). Note "no quantitative locus found".

### S2C — two-tier (work in order)

**Tier 1 — Dimension cuts in parallel:**
- \`language\` × S2C — single language → localised select-page issue
- \`device_type\` × S2C — mobile → select-page UX/rendering issue
- \`experience_id\` × S2C — specific experiences → supply/pricing. Confirm with \`count_days_available_30d\`; run \`inventory_availability\` lead-time bucket query to identify which window went empty.

If language/device concentrates: drill to intersection, then to \`page_url\`.

If experience concentrates: demand-side (\`lead_time_days\` distribution) + supply-side (\`inventory_availability\` bucket) — both shifting in same window = supply-caused.

**Tier 2 — If no concentration:** Did checkout flow or variant selection UI change? Did availability drop uniformly (platform-wide config change)?
...
1. Pax availability at checkout — certain pax configurations unavailable → DRI: Ops/BDM
2. Price display friction — total checkout price differs from listing price
...
2. Fraud rule tightening — new fraud model blocking more legitimate transactions
